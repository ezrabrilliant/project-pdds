...
const GenreExplorer: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'total_count' | 'movies_count' | 'tv_shows_count'>('name');
  ...
  const sortedGenres = [...genres].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'tv_shows_count':
        return b.tv_shows_count - a.tv_shows_count;
      default:
        return 0;
    }
  });
      ...
      <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-2xl font-bold text-white">
                {genres.reduce((sum, genre) => sum + genre.movies_count, 0).toLocaleString()}
              </p>
        ...
        <div className="bg-gradient-to-r from-green-600/20 to-blue-600/20 rounded-xl p-6 border border-green-500/20">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-2xl font-bold text-white">
                {genres.reduce((sum, genre) => sum + genre.tv_shows_count, 0).toLocaleString()}
              </p>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-3 bg-slate-700/30 rounded-lg">
                  ...
                  <p className="text-lg font-semibold text-white">
                    {genre.movies_count.toLocaleString()}
                  </p>
                </div>
                ...
                <div className="text-center p-3 bg-slate-700/30 rounded-lg">
                  ...
                  <p className="text-lg font-semibold text-white">
                    {genre.tv_shows_count.toLocaleString()}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs text-slate-400">
                  ...
                  <span>{((genre.movies_count / genre.total_count) * 100).toFixed(0)}% Movies</span>
                </div>
                <div className="w-full bg-slate-700 rounded-full h-2">
                  <div
                    className={`bg-gradient-to-r ${getGenreColor(index)} h-2 rounded-full transition-all duration-300`}
                    style={{ width: `${(genre.movies_count / genre.total_count) * 100}%` }}
                  ></div>
                </div>
              </div>
      ...
      <div className="bg-gradient-to-r from-slate-800/50 to-purple-900/30 backdrop-blur-sm rounded-2xl p-8 border border-purple-500/20">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-slate-300">
              {sortedGenres.sort((a, b) => b.movies_count - a.movies_count)[0]?.name || 'N/A'}
            </p>
            <p className="text-sm text-slate-400">
              {sortedGenres.sort((a, b) => b.movies_count - a.movies_count)[0]?.movies_count.toLocaleString()} movies
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-slate-300">
              {sortedGenres.sort((a, b) => b.tv_shows_count - a.tv_shows_count)[0]?.name || 'N/A'}
            </p>
            <p className="text-sm text-slate-400">
              {sortedGenres.sort((a, b) => b.tv_shows_count - a.tv_shows_count)[0]?.tv_shows_count.toLocaleString()} shows
            </p>
          </div>
        </div>
      </div>
